@foreach($achievements as $achievement)
    ...
    <div class='link'>
        <a href="{{route('achievement.show', ['id'=>$achievement->id])}}"><div>{{$achievement->name}} ({{$achievement->score}})</div></a>
    </div>
@endforeach
...
@if ($achievementDoesNotExist)
    <form class='link' method="POST" action="{{route('achievement.store')}}">
        {{csrf_field()}}
        <input type='hidden' name='name' value='{{$searchQuery}}' />
        <input type='submit' value="Create Achievement &quot;{{$searchQuery}}&quot;" />
    </form>
@endif
@if (count($achievements)==1)
    <input type='hidden' id='default_result' value="{{route('achievement.show', ['id'=>$achievement->id])}}" />
@else
